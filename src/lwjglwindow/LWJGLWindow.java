...
public class LWJGLWindow 
{
	...
	public LWJGLWindow(String name, int x, int y, IUpdater u, IDrawer d, boolean vsync)
	{
		this.name = name;
	...
	public void fillLine(double x, double y, double x1, double y1, double thickness)
	{
		double angle = Math.atan2(y1 - y, x1 - x);

		double oy = Math.cos(angle) * thickness / 2;
		double ox = -Math.sin(angle) * thickness / 2;

		glBegin(GL_TRIANGLE_FAN);
		glVertex2d(x - ox, y - oy);
		glVertex2d(x + ox, y + oy);
		glVertex2d(x1 + ox, y1 + oy);
		glVertex2d(x1 - ox, y1 - oy);
